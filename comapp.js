...
(function () {
    ...
    var init = function () {
        ...
        $root.$on("geSlotUpdated", function (_, data) {
            if (data.offerCompletedCount > 0 && !data.isAborted && data.offerItem !== undefined) {
                var id = data.offerItem.id;
                var price = data.offerCompletedGold / data.offerCompletedCount;
                var selling = data[Object.keys(data)[2]] == 1;
        ...
        $root.$on("itemDetailsRetrieved", function (_, objdata) {
            var id = objdata.id;
            objdata.lastSell = loadTradeHistory(id + "|Selling");
            objdata.lastBuy = loadTradeHistory(id + "|Buying");
        });
